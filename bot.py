...
def fetch_and_filter_games():
    ...
    try:
        ...
        for item in fixtures:
            ...
            # TIMEZONE CORRECTION (UTC to Brasília Time: -3 hours)
            ...
            is_vip_game = league_name in ["Premier League", "La Liga", "Serie A", "Champions League", "Serie A - Brazil", "Serie A"]
            ...
            game_payload = {
                ...
                "next_day": False,
                "is_vip": is_vip_game,
                "analysis": "Beta analysis: Automated high-volume trend selection."
            }
...
def push_to_github(content):
    ...
    # Get the current file SHA to overwrite it
    ...
    payload = {"message": "Auto-update: Daily Match Matrix", "content": content_b64}
    if sha:
        payload["sha"] = sha
        
    requests.put(url, headers=headers, json=payload)

# Streamlit Interface (Just to trigger or monitor)
    ...
    with st.spinner("Processing games and updating server..."):
        ...
        if games:
            output_data = {
                ...
                "assertiveness": "84%", "greens": "0", "reds": "0",
                "current_gate_key": CURRENT_MONTH_PASSWORD,
                "analyzed_games": games
            }
            push_to_github(output_data)
            st.success(f"Done! {len(games)} games pushed to Vercel with correct Brasília time.")
        else:
